...
class BugStatusesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @bug_status.save
        flash[:notice] = 'Новый статус ошибок создан.'
        format.html { redirect_to(admin_bug_status_path(@bug_status)) }
  ...
  def update
    ...
    respond_to do |format|
      if @bug_status.update_attributes(params[:bug_status])
        flash[:notice] = 'Статус ошибок успешно изменен.'
        format.html { redirect_to(admin_bug_status_path(@bug_status)) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(admin_bug_statuses_url) }
      format.xml  { head :ok }
    end
  end
end
