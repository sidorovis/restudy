...
class IssueStatusesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @issue_status.save
        flash[:notice] = 'Статус новых возможностей создан.'
        format.html { redirect_to(admin_issue_status_path(@issue_status)) }
  ...
  def update
    ...
    respond_to do |format|
      if @issue_status.update_attributes(params[:issue_status])
        flash[:notice] = 'Статус новых возможностей изменен.'
        format.html { redirect_to(admin_issue_status_path(@issue_status)) }
        format.xml  { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to(admin_issue_statuses_url) }
      format.xml  { head :ok }
    end
  end
end
